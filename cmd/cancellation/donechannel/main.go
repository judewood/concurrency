...
func liftCups(cups []cup) int {
	doneCh := make(chan struct{}) //channel of empty structs requires no allocation
...
Loop:
	for _, liftedCup := range cups {
		...
		default:
			...
			go func() {
				defer wg.Done()
				winningCup := checkCup(liftedCup, doneCh)
				if winningCup {
					winner = liftedCup.ID
				}
			}()
...
func checkCup(liftedCup cup, doneCh chan struct{}) bool {
	hasCoin := false
	fmt.Printf("\nLifting cup %d...", liftedCup.ID)
	if liftedCup.value == coin {
		hasCoin = true
		doneCh <- struct{}{} //signal to stop lifting any more cups
	}
	return hasCoin
}
